...
def rescue_error_and_retry(max_retries = 30)
  retries = 0
  begin
    yield
...
def setup_topology_and_client(orchestration, preset, id = nil)
  ...
  case orchestration
    when 'servers'
      @mongodb_uri = @topology.object['mongodb_uri'] + '/' + TEST_DB
      @client = Mongo::Client.new(@mongodb_uri, mode: :standalone)
      @n = 1
    when 'replica_sets'
      @mongodb_uri = @topology.object['mongodb_uri'].sub(%r{/\?}, "/#{TEST_DB}?")
    ...
    when 'sharded_clusters'
      @mongodb_uri = @topology.object['mongodb_uri'] + '/' + TEST_DB
      @client = Mongo::Client.new(@mongodb_uri, mode: :sharded)
...
When(/^I insert a document with retries$/) do
  @result = rescue_error_and_retry do
    @coll.insert_one({a: @ordinal})
  end
end
...
When(/^I query with read\-preference (\w+)$/) do |read_preference|
  read_preference_sym = read_preference.downcase.to_sym
  @result = with_rescue do
    find_one_ordinal(mode: read_preference_sym)
  end
end
...
When(/^I query with read\-preference (\w+) and tag sets (.*)$/) do |read_preference, tag_sets|
  ...
  read_preference_sym = read_preference.downcase.to_sym
  @result = with_rescue do
    find_one_ordinal(mode: read_preference_sym, tags: @tag_sets)
  end
end
